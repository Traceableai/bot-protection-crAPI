...
@Slf4j
@Service
public class TraceableCaptchaValidateTokenServiceImpl implements TraceableCaptchaValidateTokenService {
    ...
    public ResponseEntity<?> validateToken(String token, String remoteIp) {
        try {
            ...
            // Parse the response
            TraceableCaptchaResponse captchaResponse = objectMapper.readValue(
                    response.getBody(), 
                    TraceableCaptchaResponse.class
            );
